...
| `PUBLIC_ERROR_UNUSUAL_ACTIVITY` | Google rejected the generation as unusual activity / reCAPTCHA risk evaluation | **Do not auto-retry** — repeated submits can prolong the block | Pause generation submits for a few minutes, ensure no VPN/proxy is in use, then retry manually. If it persists, refresh the Flow login/session and wait before trying again |
